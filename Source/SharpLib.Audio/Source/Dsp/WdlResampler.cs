...
using WDL_ResampleSample = System.Single;
using WDL_SincFilterSample = System.Single;
...
namespace SharpLib.Audio.Dsp
{
    internal class WdlResampler
    {
        ...
        private WDL_SincFilterSample[] m_filter_coeffs;
        ...
        private WDL_ResampleSample[] m_rsinbuf;
        ...
        public void SetMode(bool interp, int filtercnt, bool sinc, int sinc_size = 64, int sinc_interpsize = 32)
        {
            ...
            if (m_sincsize == 0)
            {
                m_filter_coeffs = new WDL_SincFilterSample[0];
                m_filter_coeffs_size = 0;
            }
            if (m_filtercnt == 0)
            {
        ...
        private void BuildLowPass(double filtpos)
        {
                ...
                m_lp_oversize != wantinterp)
            {
                ...
                if (m_filter_coeffs.Length == allocsize)
                {
                    ...
                    double dsincpos = PI / m_lp_oversize * filtpos;
                    double sincpos = dsincpos * -hsz;

                    int x;
                    for (x = -hsz; x < hsz + m_lp_oversize; x++)
                    {
                        double val = 0.35875 - 0.48829 * Math.Cos(windowpos) + 0.14128 * Math.Cos(2 * windowpos) - 0.01168 * Math.Cos(6 * windowpos);
                        if (x != 0)
                        {
        ...
        private void SincSample(WDL_ResampleSample[] outBuffer,
            int outBufferIndex,
        ...
        private void SincSample1(WDL_ResampleSample[] outBuffer,
            int outBufferIndex,
            WDL_ResampleSample[] inBuffer,
            int inBufferIndex,
            double fracpos,
        ...
        private void SincSample2(WDL_ResampleSample[] outptr,
            int outBufferIndex,
            WDL_ResampleSample[] inBuffer,
            int inBufferIndex,
            double fracpos,
        ...
        #region Вложенный класс: WDL_Resampler_IIRFilter
