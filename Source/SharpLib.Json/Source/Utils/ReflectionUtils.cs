    ...
    internal static class ReflectionUtils
    {
        ...
        public static object GetMemberValue(MemberInfo member, object target)
        {
            ...
            switch (member.MemberType())
            {
                ...
                default:
                    throw new ArgumentException("MemberInfo '{0}' is not of type FieldInfo or PropertyInfo".FormatWith(CultureInfo.InvariantCulture, CultureInfo.InvariantCulture, member.Name),
                        "member");
            }
        }
        ...
        {
            const BindingFlags BINDING_ATTR = BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic;
            ...
            switch (memberInfo.MemberType())
            {
                case MemberTypes.Property:
                    ...
                    return targetType.GetProperty(propertyInfo.Name, BINDING_ATTR, null, propertyInfo.PropertyType, types, null);
                default:
                    return targetType.GetMember(memberInfo.Name, memberInfo.MemberType(), BINDING_ATTR).SingleOrDefault();
            }
        }
            ...
            while ((targetType = targetType.BaseType()) != null)
            {
                foreach (PropertyInfo propertyInfo in targetType.GetProperties(bindingAttr))
                {
                    ...
                    else
                    {
                        ...
                        {
                            int index = initialProperties.IndexOf(p =>
                            {
                                var declaringType = p.GetBaseDefinition().DeclaringType;
                                return declaringType != null && (p.Name == subTypeProperty.Name
                                                                 && p.IsVirtual()
                                                                 && p.GetBaseDefinition() != null
                                                                 && declaringType.IsAssignableFrom(subTypeProperty.DeclaringType));
                            });
