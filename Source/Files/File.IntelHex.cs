...
namespace SharpLib
{
    ...
    public class IntelHexRecord
    {
        ...
        public UInt16 Addr
        {
            get { return _buffer.GetByte16Ex(1, Endianess.Big); }
        }

        public IntexHexRecordTyp Typ
        {
            get { return (IntexHexRecordTyp)_buffer[3]; }
        }
        ...
        public Byte Crc
        {
            get { return _buffer[_buffer.Length - 1]; }
        }
        ...
        {
            get { return (Typ == IntexHexRecordTyp.LinearAddr || Typ == IntexHexRecordTyp.SegmentAddr); }
        }

        public Boolean IsData
        {
            get { return (Typ == IntexHexRecordTyp.Data); }
        }
    ...
    public class IntelHexRegion : IComparable
    {
        ...
        {
            get { return (AddrStart + REGION_SIZE - 1); }
        }
    ...
    public class IntelHex
    {
        ...
        public Boolean LoadFile(String filename)
        {
            ...
            return LoadFile(filename, out error);
        }

        public Boolean LoadFile(String filename, out String error)
        {
            error = "";
            ...
            Boolean result = LoadText(text, out error);

            return result;
        }

        public Boolean LoadText(String text)
        {
            String error;

            return LoadText(text, out error);
        }

        public Boolean LoadText(String text, out String error)
        {
            error = "";
            ...
            if (lines.Length == 0)
            {
                error = "Файл не IntelHex формата";
                return false;
            }
            ...
            if (Regions.Count == 0)
            {
                error = String.Format("Не найдено ни одного региона");
                return false;
            }
        ...
        public Boolean SaveFile(String filename, out String error, int width = SAVE_HEX_WIDTH)
        {
            if (Regions.Count == 0)
            {
                error = "Нет регионов с данными";
                return false;
            }
        ...
        public Byte[] ReadBuffer(UInt32 addr, int size, out String error)
        {
            ...
            if (region == null)
            {
                error = IntelHexRegion.ErrorAddrText(addr);
                return null;
            }
